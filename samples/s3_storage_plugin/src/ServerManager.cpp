...
std::string ServerManager::getServerID()
{
    DEBUGLOG("ServerManager::getServerID");
    std::string server;
    CURL* curl = curl_easy_init();
    if (!curl) 
    {
    ...
    {
        std::string url = "https://" + m_host +  "/rest/v2/servers";
        DEBUGLOG("url",url);
        curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
        curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "GET");
        ...
        else 
        {
            ...
            if(validJson && jsonData.isArray())
            {
                for (const auto& jsonObject : jsonData) 
                {
                    if(jsonObject.isMember("id"))
                    {
                        server = jsonObject["id"].asString();
                        server.erase(server.begin(),server.begin()+1);
                        server.erase(server.end()-1,server.end());
                        INFOLOG("--server----->",server);
                        break;
                    }
                }
            }
