...
{
	// std::cout << __LINE__ << ":" << __func__ << " : " << "ftplib::Put  " 
	// << "localfile:" << localfile
	// << ",path:" << path
	// << ",type:" << type 
	// << ",mode:" << mode << std::endl;
	...
	if (localfile != NULL)
	{
		...
		if (local == NULL)
		{
			std::cout << __LINE__ << ":" << __func__ << " : " << "file not exist:" <<  localfile << std::endl;
			strncpy(nControl->response, strerror(errno), sizeof(nControl->response));
			return 0;
		}
	...
	if (!FtpAccess(path, type, mode, nControl, &nData)) 
	{
			if (localfile != NULL) 
				fclose(local);
			std::cout << __LINE__ << ":" << __func__ << " : " << "FtpAccess failed:" <<  path << std::endl;
			return 0;
  	}
